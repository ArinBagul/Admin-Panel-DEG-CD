...
function Pso() {
        ...
        Object.keys(dataSnapshot).forEach((psoKey) => {
          ...
          Object.keys(psoData).forEach((key)=>{
            formattedData.push({
              ...
              designation: psoData[key].designation,
  ...
  const handleDelete = async (psoID, id) => {
    try {
      console.log(id);
      await remove(ref(db, `pso/${psoID}/${id}`)); // Remove the item from Firebase
          ...
          <tbody>
            {data.map((item, index) => (
              <tr key={index}>
                <td>{item.name}</td>
                <td>{item.psName}</td>
                <td>{item.psAddress}</td>
                <td>{item.mobileNumber}</td>
                <td>{item.designation}</td>
                <td>
                  <button
                    className="deleteBtn"
                    onClick={() => handleDelete(item.psoID, item.id)}
                  >
                    <svg
