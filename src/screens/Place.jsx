...
const Place = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const districtRef = ref(db, "district"); // Assuming 'district' is Firebase database node
      ...
      try {
        ...
        Object.keys(dataSnapshot).forEach((districtKey) => {
          const districtData = dataSnapshot[districtKey];
          Object.keys(districtData).forEach((placeKey) => {
            const placeData = districtData[placeKey];
            formattedData.push({
              id: placeKey,
  ...
  const handleDelete = async (district, id) => {
    try {
      console.log(id);
      await remove(ref(db, `district/${district}/${id}`)); // Remove the item from Firebase
